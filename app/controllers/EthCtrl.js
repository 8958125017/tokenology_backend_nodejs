...
var merchandise = require('./merchandise.js');
...
var fs = require('fs');
var contract = JSON.parse(fs.readFileSync(__dirname+'/json/contract.json', 'utf8'));
var crowdsale = JSON.parse(fs.readFileSync(__dirname+'/json/crowdsale.json', 'utf8'));

// var ini = require('ini');
// var configuration = ini.parse(fs.readFileSync('../kryptual.config.ini', 'utf-8'));
// if(configuration.connectETHnodeWS){
//  console.log("from eth:",configuration.connectETHnodeWS);
// }
// else{
// console.log("else:");	
// }
	 	...
	 	web3.eth.sendTransaction({from: fromAddress, to:toAddress, value: web3.utils.toWei('1', 'ether'),
	 	 gasLimit: 21000, gasPrice: 20000000000})
			...
			.on('error', function(err){
				...
				res.send({status : 400, message : "Oop's transaction failed. Please try agian!.",error : err });
			});
...
var withdrawEth = function(req, res){
	 ...
	 var isAddressValid = web3.utils.checkAddressChecksum(toAddress);
	 if(isAddressValid){
	  ...
	  User.findOne({ _id: userId},{}, function (err, data) {
                    ...
                    if(data){
                    	 ...
                    	 bcrypt.compare(password, data.password, function (err, result) {
                                    if (err) {
                                        ...
                                        if (result === true) {
                                        		 web3.eth.personal.unlockAccount(fromAddress,data.email, 1000);  //unlock by password
                                        		 // web3.eth.personal.unlockAccount(fromAddress,data.email, 15000);
                                        		 console.log('**isAccountUnlock::',fromAddress,web3.eth.personal.unlockAccount(fromAddress,data.email, 15000));

													 if(fromAddress && toAddress){
													 	web3.eth.getGasPrice().then(function(gasPrice){

														let txObj = {from: fromAddress, to:toAddress, value: web3.utils.toWei(amount, 'ether'),
														gasLimit: 21000, gasPrice: gasPrice};
													 	...
													 	web3.eth.sendTransaction(txObj)
													 	   .on('transactionHash', function(hash){
													 	   	...
													 	   	res.send({status : 200, transactionHash : hash, message : 'ETH withraw successfully!.' });
													 	   		...
													 	   		web3.eth.personal.lockAccount(fromAddress,data.email);

													 	   		console.log("lock after transaction");
                                            ...
                                            return res.send({message: 'Authentication failed due to wrong details.',status:400});
                                        }
                                    }
                        });
                    }
		...
		return res.send({message: 'Invalid address, Please try again!.',status:400});
	}
}

var purchaseProducts = function(req, res){
	 let fromAddress = req.body.fromAddress;
	 let toAddress = req.body.toAddress;
	 let amount = req.body.amount;
	 let userId = req.body.userId;
	 let password = req.body.password;
	 // let isPointUsed = req.body.isPointUsed;
                                    ...
                                    if (err) {
                                       return  res.send({message: err,status:400});
                                        ...
                                        if (result === true) {
                                        		 ...
                                        		 web3.eth.personal.unlockAccount(fromAddress,data.email, 1000);
													 	...
													 	web3.eth.sendTransaction({from: fromAddress, to:toAddress, value: web3.utils.toWei(amount, 'ether'),
													 	 gasLimit: 21000, gasPrice: 20000000000})
													 	   .on('transactionHash', function(hash){
													 	   	console.log('transactionHash:::::::::'+hash);
													 	   	web3.eth.personal.lockAccount(fromAddress,data.email);
													 	   	// if(isPointUsed && points){
														 	   // 	User.updateOne({_id : userId}, {$set : {"kryptualPoints" : data.kryptualPoints - points }},function(err, updated){
														 	   // 		if(!updated) return res.send({status : 400, message : 'Points not updated' }); 
														 	   // 		if(updated) return res.send({status : 200, transactionHash : hash, message : 'You purchased items successfully!.' });
														 	   // 	});
													 	    // }

													 	   	// res.send({status : 200, transactionHash : hash, message : 'ETH withraw successfully!.' });
													 	   		merchandise.buy(req,res,data,function(err,result){
																	if(err){
																		console.log("**error in buy**");
																		return res.send({status : 400, message:"error while buying.",err : err})	
																	}
																	if(!result){
																		console.log("**error while buy mail**");
																		return res.send({status : 400, message:"error while sending an email!."})
																	}
																	if(result){
																		console.log("**nice for buy**");
																		return res.send({status : 200, message:"Thanks for shopping with KRYPTUAL!."})
																	}
																});

																// Mail.withdrawEthMail(req.body,data,function(msg){
																// 	console.log("the mail send sucessfully for withdraw while purchasing products!.",data);
																// })
																
															})
															.on('receipt', function(receipt){
																console.log('receipt'+receipt);
																return res.send({status : 200, receipt : receipt });
															})
															.on('confirmation', function(confirmationNumber, receipt){
																console.log('confirmationNumber'+receipt);
																return res.send({status : 200, confirmationNumber : confirmationNumber, receipt:receipt });
															 })
															.on('error', function(err){
																return res.send({status : 400, message : 'Insufficient balance',error : err });
															});

													 }else{
													 	return res.send({status : 200, message : 'Please send required parameter'});
													 }
                                        }
                                        else {
                                            return res.send({message: 'Authentication failed due to wrong details.',status:400});
...
var savedICOtokenIntoDB = function(req, res){
	console.log('*****api hitiing***** savedICOtokenIntoDB',req.body);
	...
	let investorMinCap = req.body.token.globalmincap;
	...
	//  Additional for rate
	let pricingStategy = req.body.pricingStrategy ? req.body.pricingStrategy : [{rate : 0}];
	let investorMinimumCap = req.body.investorMinCap ? req.body.investorMinCap : 0;
  ...
  //       let crowdsaleCounter = 0;
	 //    let totalsupply = 0;
	 // 	Async.forEachLimit(crowdsale,1,(crowdsalelement,next)=>{

	 // 		console.log('**find multiple crowdsale**',crowdsale,crowdsaleCounter);
	 // 		crowdsaleCounter++;
	 // 		totalsupply = totalsupply + parseInt(crowdsalelement.supply);
	 		
	 // 		if(crowdsaleCounter < crowdsale.length){
	 // 			crowdsale[crowdsaleCounter].rate = pricingStategy[crowdsaleCounter].rate;
	 // 			next();
	 // 			console.log("**counter**",crowdsaleCounter,crowdsale.length);
	 // 		}else{
	 // 			tokenSupply = totalsupply;
	 // 			crowdsale[crowdsaleCounter].rate = pricingStategy[crowdsaleCounter].rate;
	 // 			console.log("**update final**",crowdsale[crowdsaleCounter].rate);
	 // 		}
	 // 		console.log('*increasing counter*',pricingStrategy);
	 // });
	...
	if(userId){
		...
		let ICOOBJ = {
				...
				investorMinimumCap : investorMinimumCap,
				pricingStategy : pricingStategy
			};
				ICO.findOne({ crowdsaleAddress : crowdsaleAddress},{},function(err, get){
					
					if(err) return res.send({status : 400, message : 'Someting went wrong!.'});
					if(!get){
						// new ICO entry into DB
						ICO.create(ICOOBJ, function(err, ico){
						...
						// ICO already saved into DB
						return res.send({status : 400, message : 'ICO already saved into DB'});
					}
...
var sendTokenToUser = function(req, res){
    ...
    let fromAddress = req.body.fromAddress;
    let toAddress = req.body.toAddress;
    let tokenContractAddress = req.body.tokenContractAddress;
    let amount = req.body.amount;
    let userId = req.body.userId;

     var isAddressValid = web3.utils.checkAddressChecksum(toAddress);
	if(isAddressValid){
		  var Contract = new web3.eth.Contract(contract, tokenContractAddress);

		  User.findOne({ _id: userId},{}, function (err, data) {
		        console.log('sendTokenToUser params : ',req.body);
		        if (err) {
		            res.send({message: err,status:400});
		        } else {
		            if(data){
					     ...
					     bcrypt.compare(password, data.password, function (err, result) {
					        if (err) {
					            ...
					            if (result === true) {
								    web3.eth.personal.unlockAccount(fromAddress,data.email, 1000);
											  ...
											  Contract.methods.transfer(toAddress,amount).send(opts)
											  .on('receipt', function(receipt){
											      console.log('receipt',receipt);
											  })
											.once('transactionHash', function (txHash) {
												web3.eth.personal.lockAccount(fromAddress,data.email);
											  return res.send({status : 200, data : txHash, message : amount+" Token send successfully!."});
							...
							return res.send({status : 400, message : 'User does not exists!.'});
						}
					}
				});						
    }
    else{
    	return res.send({status : 400, message : "Invalid address, Please try again!."});
    }
}

var buyTokenToUser = function(req, res){
    let password = req.body.password;
    let fromAddress = req.body.fromAddress;
    let crowdsaleAddress = req.body.crowdsaleAddress;
    let amount = req.body.amount;
 ...
 var isAddressValid = web3.utils.checkAddressChecksum(crowdsaleAddress);
	if(isAddressValid){
	  var Crowdsale = new web3.eth.Contract(crowdsale, crowdsaleAddress);
	  User.findOne({ _id: userId},{}, function (err, data) {
	        console.log('buyTokenToUser params : ',req.body);
	        if (err) {
	            res.send({message: err,status:400});
	        } else {
	            if(data){
				        ...
				        if (err) {
				            ...
				            if (result === true) {
							    web3.eth.personal.unlockAccount(fromAddress,data.email,1000);
									  ...
									  Crowdsale.methods.buy().send(opts)
									  .on('receipt', function(receipt){
									      console.log('receipt',receipt);
									    })
									  .once('transactionHash', function (txHash) {
									  	console.log('transactionHash', txHash);
									  	web3.eth.personal.lockAccount(fromAddress,data.email);
									  	return res.send({status : 200, data : txHash, message : "You have bought "+amount+" Token send successfully!."});
									})
									.once('confirmation', function(confirmationNumber, receipt) {
									  console.log('confirmation', confirmationNumber, receipt);
									})
...
var makeCrowdsaleFinal = function(req, res){
    let fromAddress = req.body.fromAddress;
    let crowdsaleAddress = req.body.crowdsaleAddress;
    let userId = req.body.userId;
 ...
 var isAddressValid = web3.utils.checkAddressChecksum(crowdsaleAddress);
	if(isAddressValid){
	  var Crowdsale = new web3.eth.Contract(crowdsale, crowdsaleAddress);
	  User.findOne({ _id: userId},{}, function (err, data) {
	        console.log('makeCrowdsaleFinal params : ',req.body);
	        if (err) {
	            res.send({message: err,status:400});
	        } else {
	            if(data){
				    web3.eth.personal.unlockAccount(fromAddress,data.email,1000);
						 ...
						 Crowdsale.methods.finalize().send(opts).on('receipt', function(receipt){
						      console.log('receipt',receipt);
						    })
						  .once('transactionHash', function (txHash) {
						  	console.log('transactionHash', txHash);
						  	web3.eth.personal.lockAccount(fromAddress,data.email);
						  	return res.send({status : 200, data : txHash, message : "Crowdsale has been finalized!."});
						})
						.once('confirmation', function(confirmationNumber, receipt) {
						  console.log('confirmation', confirmationNumber, receipt);
						})
...
var readAbi = function(req, res){
    ...
    var MyContract = new web3.eth.Contract(Constant.dummyAbi,crowdSaleAddress);
    console.log("**MyContractMyContract,",MyContract);

    MyContract.methods.totalSupply.call(function(err,data){
       return res.send({status :200, tokenBalance : data , message : 'Get Token Balance', err:err});
    });
}
...
exports.buyTokenToUser = buyTokenToUser;
exports.makeCrowdsaleFinal = makeCrowdsaleFinal;