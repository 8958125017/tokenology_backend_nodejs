...
var listICO = (req,res)=> {
 ...
 var isAddressValidToken = web3.utils.checkAddressChecksum(tokenAddress);

    if(isAddressValid && isAddressValidToken){
        ...
        .then((success)=>{
            if(success)
            {
                ...
                text += otppossible.charAt(Math.floor(Math.random() * otppossible.length));
                    ...
                    if(i>=6)
                    {
                        User.findOneAndUpdate({_id:req.body.userId},{$set:{'referralReseller':req.body.code,'referralUser': 'KRYPTUAL'+text}},{new:true})
                        .then((success)=>{
                            if(success)
                                return res.send({status : 200, status : true, message :'Reseller code generated successfully!.',data:success.referralUser})
                                 else
