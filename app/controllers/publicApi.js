            ...
            User.findOne({email: user.email.toLowerCase()},{}, function (err,data) {
                if (err) {
                    ...
                    crypto.randomBytes(10, function (err, buf) {
                        ...
                        let referralUser = req.body.referralUser;
                        ...
                        if(accountType==='Investor'){
                            ...
                            if(investorCode!=''){
                                ...
                                User.findOne({ generatedInvestorCode : investorCode},{},function(err,getData){
                                    ...
                                    if(getData){
                                        ...
                                        return res.send({status : 400, message : 'Invalid referral code!.'})
                                    }    
                                });
                            ...
                            if(referralUser!=''){
                                console.log('***Have code***');
                                User.findOne({ referralUser : referralUser},{},function(err,getData){
                                    if(err) return res.send({status : 400, message : 'error while fetch'})
                                    if(getData){
                                        registration(req, res,user);
                                        return false;
...
function registration(req, res,user){
    //For generate ETHRIUM Address
                        ...
                        web3.eth.personal.newAccount(user.email.toLowerCase()).then((data) => {
                            // var data = web3.eth.accounts.create(user.password);
                            console.log('***creating ETHERUM ADDRRESS****',data);
                            if(data){
                                user.EthAddress = data;
                                // user.EthAddress = data.address;
...
var login = function (req, res) {
    ...
    if (!user || !user.email) {
        ...
        User.findOne({email: user.email.toLowerCase()},
            {}, function (err, data) {
                console.log('login2',data,err,req.body);
                if (err) {
  ...
  if(req.body.email.toLowerCase()){
     Contact.create(reqObj,function(err,data){
           if(err){
            ...
            res.json({ message : "Your query sent successfully to Kryptual team!.", status : 200 })
    ...
    if (userId) {
        User.findOne({ _id: userId }, {}, function(err, data) {
            if (err) {
                ...
                else return res.json({message: "You have not generated your referral code, yet!.",status: 400, data : ''})
            }
            else {
...
// for reseller code getting
var getResellerCode = (req, res) => {
    let userId = req.body.userId;
    if (userId) {
        User.findOne({ _id: userId }, {}, function(err, data) {
            if (err) {
                ...
                if(data.referralUser){
                    console.log("****generatedInvestorCode***",data.referralUser);
                    return res.json({message: "Your refferal code is : " + data.referralUser,status: 200,data : data.referralUser})
                }
                else return res.json({message: "You have not generated your reseller code, yet!.",status: 400, data : ''})
            }
            else {
                return res.json({message: "Please enter the valid user id ",status: 400 });
            }
...
var getKryptualPoints = (req, res) => {
    ...
    if (userId) {
        ...
        return res.send({data : [],status : 400, message: "User does not exists!."})
    }
}

var createAddress = function(req,res){
    var data = web3.eth.accounts.create('kryptual');
    return res.send({status : 200, data : data});
}

var validateAddress = function(req, res){
    let address = req.body.address;

    var isAddressValid = web3.utils.checkAddressChecksum(address);
     if(isAddressValid){
        return res.send({status : 200, message: "Valid address!."})
     }else{
       return res.send({status : 400, message: "Invalid address!."}) 
     }
}
...
exports.getResellerCode = getResellerCode;
