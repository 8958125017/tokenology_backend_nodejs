...
'getProductById':(req,res)=>{
  ...
  Product.find({status:true,_id : productId})
  .then((success)=>{
     if(success)
    res.json({statusCode : 200 , message : 'Products fetch successfully.', data:success})
  else
...
'buy':(req,res,userData,callback)=>{
  let err = 'Something went wrong!';
    // if(!req.body.user_id || !req.body.address || !req.body.product){
  if(!req.body.user_id || !req.body.product){
       // return res.json({statusCode : 402 , message : 'Please fill the required fields.'})
     callback(err,null);
     console.log('****buy data::',req.body);
   }
      else
       {
          Order.create({
           ...
           if(success){
            console.log("****save order!**");
             // res.json({statusCode : 200 , message : 'Products ordered successfully.', data:success})
              Mail.buyproduct(req.body,userData,success,function(msg){
               console.log("the mail send sucessfully for buy product",msg);
               callback(null,success);
            });
          }
          else{
            console.log("****not succes on data save!**");
            callback(err,null);
          }
