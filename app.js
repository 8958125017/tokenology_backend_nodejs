  ...
  var ini = require('ini');
  var fs = require('fs');
  var config = ini.parse(fs.readFileSync('../kryptual.config.ini', 'utf-8'));
  ...
  var port = normalizePort(process.env.PORT || config.backendPort);
  app.set('port', port);
...
// mongodb://admin:pmyfe423ct5ef4yh@localhost:27017/tokonology?authSource=admin
  // mongodb://admin:pmyfe423ct5ef4yh@52.66.185.83:27017/tokonology?authSource=admin
// mongodb://localhost:27017/tokonology --for local DB 

// let databaseConnectionURL = "mongodb://admin:pmyfe423ct5ef4yh@52.66.185.83:27017/tokonology?authSource=admin";
var databaseConnectionURL = config.databaseurl + config.dbuser +":"+ config.dbpassword +"@"+config.dbhost+":"+config.dbport +"/"+ config.dbname;
console.log("*****************************Connection URL:*****",databaseConnectionURL);

  mongoose.connect(databaseConnectionURL,function (err, database) {
    if (err) {
      console.log(err);
      process.exit(1);
    }
